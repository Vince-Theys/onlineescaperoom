...
const EASEL_CAM = {
  position: [-11, 4.5, 8] as [number, number, number],
  lookAt: [-11, 4.0, 1] as [number, number, number],
}
const WHEEL_CAM = {
  position: [-3, 4.0, 11] as [number, number, number],
  lookAt: [-3, 2.1, 5] as [number, number, number],
}
const TELE_CAM = {
  position: [12, 4.8, 6] as [number, number, number],
  lookAt: [12, 3.9, -1] as [number, number, number],
}
const DOOR_CAM = {
...
function drawTree(
  ...
  for (let i = 0; i < 6; i++) {
    ...
    ctx.beginPath()
    ctx.arc(cx, cy, (10 + rng() * 10) * scale, 0, Math.PI * 2)
    ctx.fill()
  }
}
...
function paintScene(
  ...
  if (style === "landscape") {
    ...
    const sx = w * 0.5,
      sy = horizon * 0.78
    const sg = ctx.createRadialGradient(sx, sy, 4, sx, sy, 70)
    ...
    ctx.lineWidth = 2
    for (let i = 0; i < 14; i++) {
      const y = horizon + 8 + rng() * (h - horizon - 40)
      ctx.beginPath()
      ctx.moveTo(rng() * w, y)
      ctx.lineTo(rng() * w * 0.3 + rng() * w * 0.4, y)
      ctx.stroke()
    }
    ctx.fillStyle = "#d8c79a"
    ctx.beginPath()
    ctx.moveTo(0, h)
    ctx.quadraticCurveTo(w * 0.5, h - 24, w, h)
    ctx.lineTo(w, h)
    ctx.closePath()
    ctx.fill()
    ...
    ctx.beginPath()
    ctx.ellipse(w * 0.4, h * 0.35, w * 0.4, h * 0.3, 0, 0, Math.PI * 2)
    ctx.fill()
    // vase
    ctx.fillStyle = "#23506b"
    ctx.beginPath()
    ctx.moveTo(w * 0.4, h * 0.66)
    ctx.bezierCurveTo(w * 0.3, h * 0.62, w * 0.3, h * 0.42, w * 0.42, h * 0.4)
    ctx.bezierCurveTo(
      w * 0.54,
      h * 0.42,
      w * 0.54,
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const fx = w * (0.34 + rng() * 0.2)
      const fy = h * (0.22 + rng() * 0.18)
      ctx.beginPath()
      ctx.arc(fx, fy, 8 + rng() * 8, 0, Math.PI * 2)
      ctx.fill()
    ...
    ctx.beginPath()
    ctx.arc(w * 0.62, h * 0.7, 18, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = "#c8a23a"
    ctx.beginPath()
    ctx.arc(w * 0.7, h * 0.73, 14, 0, Math.PI * 2)
    ctx.fill()
  } else if (style === "night") {
    vGradient(ctx, w, 0, h, [
      [0, "#0b1330"],
    ...
    for (let i = 0; i < 80; i++) {
      ctx.fillStyle = `rgba(255,255,${200 + rng() * 55},${0.5 + rng() * 0.5})`
      ctx.beginPath()
      ctx.arc(rng() * w, rng() * h * 0.7, rng() * 1.6, 0, Math.PI * 2)
      ctx.fill()
    }
    const mx = w * 0.72,
      my = h * 0.22
    const mg = ctx.createRadialGradient(mx, my, 4, mx, my, 50)
    mg.addColorStop(0, "#fff7d8")
    mg.addColorStop(1, "rgba(255,247,216,0)")
    ctx.fillStyle = mg
    ctx.beginPath()
    ...
    ctx.lineTo(0, h * 0.78)
    for (let i = 0; i <= 10; i++) {
      ctx.lineTo((i / 10) * w, h * 0.78 - Math.abs(Math.sin(i)) * 24)
    }
    ctx.lineTo(w, h)
    ctx.closePath()
    ctx.fill()
    ...
    ctx.fillStyle = pal[seed % pal.length]
    ctx.fillRect(0, 0, w, h)
    for (let i = 0; i < 9; i++) {
    ...
    // unfinished — left half painted landscape, right half charcoal sketch on raw canvas
    ctx.fillStyle = LINEN
    ctx.fillRect(0, 0, w, h)
    ctx.save()
    ctx.beginPath()
    ...
    ctx.stroke()
    ctx.beginPath()
    ctx.arc(w * 0.78, horizon * 0.55, 18, 0, Math.PI * 2)
    ctx.stroke()
    for (let i = 0; i < 4; i++) {
      ctx.beginPath()
      ctx.moveTo(w * (0.6 + i * 0.08), horizon)
      ctx.lineTo(w * (0.62 + i * 0.08), horizon + 30 + i * 8)
      ctx.stroke()
    }
    // grid registration ticks
    ctx.strokeStyle = "rgba(120,110,95,0.35)"
    for (let gx = w * 0.55; gx < w; gx += w * 0.12) {
      ctx.beginPath()
      ctx.moveTo(gx, 0)
      ctx.lineTo(gx, h)
      ctx.stroke()
...
export default function AtelierScene({
  ...
  const easelActive =
    exploring &&
    !completedObjects.has("board") &&
    activeObjects.includes("board")
  const wheelActive =
    exploring &&
    !completedObjects.has("clock") &&
    activeObjects.includes("clock")
  const teleActive =
    exploring && !completedObjects.has("book") && activeObjects.includes("book")
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      <div className="fixed inset-0 z-0">
        ...
        >
          <Suspense fallback={null}>
            <RoomCamera
              targetPos={camProps.position}
              targetLookAt={camProps.lookAt}
              exploring={exploring}
            ...
            <Easel
              active={easelActive}
              completed={completedObjects.has("board")}
              onClick={() => onObjectClick("board")}
            />
            <PottersWheel
              active={wheelActive}
              completed={completedObjects.has("clock")}
              onClick={() => onObjectClick("clock")}
            />
            <Telescope
              active={teleActive}
              completed={completedObjects.has("book")}
              onClick={() => onObjectClick("book")}
            />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 62%, rgba(20,16,8,0.3) 100%)",
        }}
      />
...
function Painting({
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      {framed && (
        <group>
          <mesh position={[0, h / 2 + fw / 2, 0.02]} castShadow>
          ...
          <mesh position={[-w / 2 - fw / 2, 0, 0.02]} castShadow>
...
function CanvasStand({
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      {/* tray ledge */}
      <mesh position={[0, trayY, 0.12]} castShadow>
        <boxGeometry args={[1.8, 0.14, 0.44]} />
        <meshStandardMaterial color={WOOD_LIGHT} roughness={0.55} />
      </mesh>
      <mesh position={[0, trayY + 0.08, 0.28]}>
        <boxGeometry args={[1.8, 0.12, 0.06]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
      </mesh>
      {/* top clamp */}
      <mesh position={[0, h + 1.7, 0.06]}>
        <boxGeometry args={[1.6, 0.14, 0.1]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
      </mesh>
    ...
    <group>
      ...
      {Array.from({ length: 19 }).map((_, i) => (
        <mesh
          ...
          position={[-ROOM_W / 2 + 1 + i * 2 + 0.05, 0.01, 0]}
        >
      ...
      <mesh position={[0, 0.18, -ROOM_D / 2 + 0.12]}>
        <boxGeometry args={[ROOM_W, 0.36, 0.18]} />
        <meshStandardMaterial color="#c4b496" roughness={0.8} />
      </mesh>
      <mesh position={[-ROOM_W / 2 + 0.12, 0.18, 0]}>
        <boxGeometry args={[0.18, 0.36, ROOM_D]} />
        <meshStandardMaterial color="#c4b496" roughness={0.8} />
      </mesh>
      <mesh position={[ROOM_W / 2 - 0.12, 0.18, 0]}>
        <boxGeometry args={[0.18, 0.36, ROOM_D]} />
      ...
      <mesh position={[0, 8.4, -ROOM_D / 2 + 0.1]}>
        <boxGeometry args={[ROOM_W, 0.12, 0.14]} />
        <meshStandardMaterial color="#bfae8e" roughness={0.8} />
      </mesh>

      {/* Ceiling */}
      <mesh position={[0, ROOM_H, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <planeGeometry args={[ROOM_W, ROOM_D]} />
        <meshStandardMaterial color="#efe9da" roughness={1} />
      </mesh>
      ...
      <group position={[3, ROOM_H - 0.05, 0]}>
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshBasicMaterial color="#eaf4fc" side={THREE.DoubleSide} />
        </mesh>
        {[-3.3, 0, 3.3].map((x) => (
          <mesh key={`v${x}`} position={[x, -0.06, 0]}>
            <boxGeometry args={[0.12, 0.16, 7]} />
            <meshStandardMaterial color={WOOD_DARK} />
          </mesh>
        ))}
        {[-2, 2].map((z) => (
          <mesh key={`h${z}`} position={[0, -0.06, z]}>
            <boxGeometry args={[10, 0.16, 0.12]} />
            <meshStandardMaterial color={WOOD_DARK} />
          </mesh>
        ))}
      ...
      <group
        position={[ROOM_W / 2 - 0.1, 6, -6]}
        rotation={[0, -Math.PI / 2, 0]}
      >
        <mesh>
          <planeGeometry args={[4.4, 5]} />
          <meshBasicMaterial color="#d6e8f4" />
        </mesh>
        {[-1.1, 0, 1.1].map((x) => (
          <mesh key={x} position={[x, 0, 0.03]}>
            <boxGeometry args={[0.1, 5, 0.06]} />
            <meshStandardMaterial color="#e6dcc6" />
          </mesh>
        ))}
        {[-1.2, 1.2].map((y) => (
          <mesh key={y} position={[0, y, 0.03]}>
            <boxGeometry args={[4.4, 0.1, 0.06]} />
            <meshStandardMaterial color="#e6dcc6" />
          </mesh>
        ))}
        <mesh position={[0, 0, -0.05]}>
          <boxGeometry args={[4.8, 5.4, 0.16]} />
          <meshStandardMaterial color="#d2c4ac" roughness={0.8} />
        </mesh>
      </group>
    </group>
  )
...
function Taboret({
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      {/* two drawers */}
      <mesh position={[0, 1.2, 0.49]}>
        <boxGeometry args={[1.4, 0.34, 0.02]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.5} />
      </mesh>
      <mesh position={[0, 0.82, 0.49]}>
        <boxGeometry args={[1.4, 0.34, 0.02]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.5} />
      </mesh>
      {[1.2, 0.82].map((y, i) => (
      ...
      {tubes.map((c, i) => (
        <group
          key={i}
          position={[-0.55 + i * 0.2, 1.58, -0.2 + (i % 2) * 0.1]}
      ...
      <group position={[0.5, 1.55, 0.25]}>
        <mesh>
          <cylinderGeometry args={[0.13, 0.11, 0.34, 14]} />
          <meshStandardMaterial
            color="#cfe0e6"
            transparent
            opacity={0.45}
        ...
        {["#b5472f", "#3f6079", "#6f8a5a", "#c8a23a"].map((c, i) => (
          <group key={i} rotation={[0.1 * (i - 2), i * 1.4, 0.1]}>
            ...
            <mesh position={[0, 0.74, 0]}>
              <cylinderGeometry args={[0.022, 0.016, 0.12, 6]} />
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.8, 0]} castShadow>
        ...
        <meshStandardMaterial color="#8a3b50" roughness={0.9} />
      </mesh>
      <mesh position={[0, 1.62, 0]}>
        <boxGeometry args={[2.6, 0.1, 1.8]} />
        <meshStandardMaterial color="#9c4760" roughness={0.9} />
      </mesh>
      <mesh position={[0, 1.0, 0.85]} rotation={[0.2, 0, 0]}>
      ...
      <group position={[-0.4, 1.67, 0]}>
        ...
        <mesh position={[0, 0.34, 0]}>
          <cylinderGeometry args={[0.12, 0.2, 0.34, 16]} />
          <meshStandardMaterial color="#2b5e7d" roughness={0.35} />
        </mesh>
        {/* branches */}
        {[-0.3, 0, 0.35].map((a, i) => (
          <mesh
            key={i}
            position={[Math.sin(a) * 0.1, 0.6, 0]}
            rotation={[a, 0, a * 1.2]}
          >
            <cylinderGeometry args={[0.01, 0.015, 0.7, 6]} />
            <meshStandardMaterial color="#5a4322" roughness={0.8} />
          </mesh>
        ))}
      </group>
      {/* fruit */}
      <mesh position={[0.5, 1.78, 0.2]} castShadow>
        <sphereGeometry args={[0.16, 16, 16]} />
        <meshStandardMaterial color="#b5472f" roughness={0.5} />
      </mesh>
      <mesh position={[0.72, 1.76, -0.1]} castShadow>
        <sphereGeometry args={[0.13, 16, 16]} />
        <meshStandardMaterial color="#c8a23a" roughness={0.5} />
      </mesh>
      <mesh position={[0.34, 1.74, -0.25]} castShadow>
        <sphereGeometry args={[0.12, 16, 16]} />
        <meshStandardMaterial color="#7d8c4a" roughness={0.5} />
      </mesh>
      {/* an old book */}
      <mesh position={[0.2, 1.7, 0.5]} rotation={[0, 0.3, 0]} castShadow>
        <boxGeometry args={[0.5, 0.1, 0.36]} />
        <meshStandardMaterial color="#5a4322" roughness={0.7} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} scale={scale}>
      <mesh position={[0, 0.38, 0]} castShadow>
        <cylinderGeometry args={[0.42, 0.3, 0.76, 18]} />
        <meshStandardMaterial color="#b06a3f" roughness={0.7} />
      </mesh>
      <mesh position={[0, 0.74, 0]}>
        <cylinderGeometry args={[0.44, 0.42, 0.1, 18]} />
        <meshStandardMaterial color="#9c5a34" roughness={0.7} />
      </mesh>
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, seatY, 0]} castShadow>
        <cylinderGeometry args={[0.52, 0.52, 0.14, 20]} />
        <meshStandardMaterial color={WOOD} roughness={0.6} />
      </mesh>
      <mesh position={[0, seatY - 0.08, 0]}>
      ...
      {/* stretchers */}
      <mesh position={[0, 0.45, legOffset]}>
        <boxGeometry args={[legOffset * 2, 0.05, 0.05]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.65} />
      </mesh>
      <mesh position={[0, 0.45, -legOffset]}>
        <boxGeometry args={[legOffset * 2, 0.05, 0.05]} />
      ...
      <mesh position={[legOffset, 0.45, 0]}>
        <boxGeometry args={[0.05, 0.05, legOffset * 2]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.65} />
      </mesh>
      <mesh position={[-legOffset, 0.45, 0]}>
        <boxGeometry args={[0.05, 0.05, legOffset * 2]} />
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.55, 0]}>
        <boxGeometry args={[width - 0.5, 0.1, depth - 0.3]} />
      ...
      {([-1, 1] as const).flatMap((sx) =>
        ([-1, 1] as const).map((sz) => (
          <mesh
            key={`${sx}-${sz}`}
            position={[sx * (width / 2 - 0.2), 0.55, sz * (depth / 2 - 0.2)]}
            castShadow
          >
      ...
      <mesh position={[0, 0.3, depth / 2 - 0.1]}>
        <boxGeometry args={[width - 0.6, 0.08, 0.08]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.65} />
      </mesh>
      <mesh position={[0, 0.3, -depth / 2 + 0.1]}>
        <boxGeometry args={[width - 0.6, 0.08, 0.08]} />
      ...
      <mesh position={[-0.6, 0.85, depth / 2 - 0.06]}>
        <boxGeometry args={[1.2, 0.26, 0.04]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
      </mesh>
      <mesh position={[-0.2, 0.9, depth / 2 - 0.02]}>
        <sphereGeometry args={[0.04, 10, 10]} />
        <meshStandardMaterial color={BRASS} metalness={0.7} roughness={0.3} />
      </mesh>

      <PaintPot
        position={[-0.9, topY + 0.12, 0.3]}
        size={0.85}
        color="#b5472f"
      />
      <PaintPot
        position={[0.2, topY + 0.12, -0.35]}
        size={0.8}
        color="#3f6079"
      />
      <PaintPot position={[1.0, topY + 0.12, 0.1]} size={0.7} color="#6f8a5a" />
      <mesh
        position={[0.6, topY + 0.05, -0.55]}
        rotation={[0, 0.4, 0]}
        castShadow
      >
      ...
      {["#b5472f", "#c8a23a", "#3f6079", "#6f8a5a", "#e6dcc2", "#8a3b50"].map(
        (c, i) => {
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      {([0.35, 0.9, 1.35] as const).map((y, i) => (
        <group key={i}>
          <mesh position={[0, y, 0.41]}>
            <boxGeometry args={[2.0, 0.38, 0.02]} />
            <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
          </mesh>
          {[-0.7, 0.7].map((x) => (
            <mesh key={x} position={[x, y, 0.44]}>
              <sphereGeometry args={[0.05, 10, 10]} />
              <meshStandardMaterial
                color={BRASS}
                metalness={0.7}
    ...
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      {[0, 1.1].map((y, i) => (
        <group key={i}>
          ...
          {[-1.5, 1.5].map((x) => (
            <mesh key={x} position={[x, y - 0.2, -0.18]} rotation={[0.6, 0, 0]}>
      ...
      {pots.map((p, i) => (
        <group key={i} position={[p.x, 0.14 + p.h / 2, 0.06]}>
          <mesh castShadow>
            <cylinderGeometry args={[p.r * 0.72, p.r, p.h, 20]} />
            <meshStandardMaterial
              color={p.c}
              roughness={0.55}
              metalness={0.05}
          ...
          <mesh position={[0, p.h / 2 - 0.02, 0]}>
            <torusGeometry args={[p.r * 0.72, 0.02, 8, 20]} />
            <meshStandardMaterial color={p.c} roughness={0.5} />
          </mesh>
          <mesh position={[0, -p.h / 2 + 0.04, 0]}>
            <torusGeometry args={[p.r * 0.42, 0.02, 8, 20]} />
            <meshStandardMaterial color={p.c} roughness={0.55} />
          </mesh>
        </group>
      ))}
      {/* a couple of bowls/mugs on the upper shelf */}
      <mesh position={[-0.8, 1.28, 0]} castShadow>
        <cylinderGeometry args={[0.26, 0.18, 0.2, 20]} />
        <meshStandardMaterial color="#b8743f" roughness={0.55} />
      </mesh>
      <mesh position={[0.5, 1.32, 0]} castShadow>
        <cylinderGeometry args={[0.15, 0.15, 0.28, 18]} />
        <meshStandardMaterial color="#cdbb9a" roughness={0.55} />
      </mesh>
      <mesh position={[0.5, 1.32, 0.16]} rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[0.09, 0.02, 8, 16]} />
        <meshStandardMaterial color="#cdbb9a" roughness={0.55} />
      </mesh>
      <mesh position={[1.05, 1.34, 0]} castShadow>
        <cylinderGeometry args={[0.12, 0.18, 0.34, 18]} />
        <meshStandardMaterial color="#7d8c6a" roughness={0.55} />
      </mesh>
      <mesh position={[-0.1, 1.38, 0]} castShadow>
        <cylinderGeometry args={[0.1, 0.14, 0.48, 18]} />
        <meshStandardMaterial color="#3a6e7d" roughness={0.55} />
      </mesh>
    </group>
  )
}
...
function DropCloth({ position }: { position: [number, number, number] }) {
  ...
  return (
    <group position={position}>
      <mesh rotation={[-Math.PI / 2, 0, 0.06]} position={[0, 0.02, 0]}>
        <planeGeometry args={[5.5, 4.5]} />
        <meshStandardMaterial color="#d9cdb2" roughness={0.98} />
      </mesh>
      {splats.map(([x, z, c], i) => (
        <mesh key={i} rotation={[-Math.PI / 2, 0, 0]} position={[x, 0.06, z]}>
          <circleGeometry args={[0.3 + (i % 3) * 0.12, 14]} />
          <meshBasicMaterial
            color={c}
            transparent
            opacity={0.7}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]} scale={scale}>
      ...
      <mesh position={[0, 0.42, 0]}>
        <boxGeometry args={[0.6, 0.08, 0.36]} />
        <meshStandardMaterial color="#cbb981" roughness={0.95} />
      </mesh>
    </group>
  )
}

function WallSplatter({
  position,
  rotation = [0, 0, 0],
  scale = 1,
  color = "#b5472f",
...
function AtelierDecor() {
  ...
  return (
    <group>
      ...
      />
      <Painting
        position={[-ROOM_W / 2 + 0.18, 3.9, -7.5]}
        rotation={Math.PI / 2}
        w={2.0}
        h={2.6}
        style="stilllife"
      ...
      <WallSplatter
        position={[-10, 4.6, -ROOM_D / 2 + 0.14]}
        color="#b5472f"
        scale={1.2}
      />
      <WallSplatter
        position={[9, 3.3, -ROOM_D / 2 + 0.14]}
        color="#3f6079"
        scale={1.0}
      />
      <WallSplatter
        position={[13, 5.4, -ROOM_D / 2 + 0.14]}
        color="#c8a23a"
        scale={0.9}
      />
      <WallSplatter
        position={[-ROOM_W / 2 + 0.14, 4.2, 6]}
        rotation={[0, Math.PI / 2, 0]}
        color="#8a3b50"
        scale={1.1}
      />
      <WallSplatter
        position={[-ROOM_W / 2 + 0.14, 2.2, -5]}
        rotation={[0, Math.PI / 2, 0]}
        color="#6f8a5a"
        scale={0.9}
      />
      <WallSplatter
        position={[ROOM_W / 2 - 0.14, 3.6, -3]}
        rotation={[0, -Math.PI / 2, 0]}
        color="#3f6079"
        scale={1.0}
      />
      <WallSplatter
        position={[ROOM_W / 2 - 0.14, 5.8, 8]}
        rotation={[0, -Math.PI / 2, 0]}
        color="#b5472f"
        scale={0.8}
      />
...
function Easel({ active, completed, onClick }: InteractiveProps) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    hoverP.current = THREE.MathUtils.damp(
      hoverP.current,
      hovered ? 1 : 0,
      8,
      delta
    ...
    for (const m of glowMats.current) {
      if (m) m.emissiveIntensity = eint
    }
    if (lightRef.current)
      lightRef.current.intensity = glowLight(
        t,
        active,
        completed,
        hoverP.current
  ...
  return (
    <group position={EASEL_POS}>
      ...
      {/* feet */}
      <mesh position={[-0.6, 0.08, 0.5]} castShadow>
        <boxGeometry args={[0.5, 0.16, 1.6]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
      </mesh>
      <mesh position={[0.6, 0.08, 0.5]} castShadow>
        <boxGeometry args={[0.5, 0.16, 1.6]} />
      ...
      <mesh position={[0, 2.7, 0.57]}>
        <boxGeometry args={[1.7, 0.18, 0.05]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
      </mesh>

      {/* adjustable top clamp bar + knobs */}
      <mesh position={[0, 5.05, 0.18]}>
        <boxGeometry args={[1.5, 0.16, 0.1]} />
        <meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
      </mesh>
      {[-0.55, 0.55].map((x) => (
        <mesh key={x} position={[x, 5.05, 0.3]} rotation={[Math.PI / 2, 0, 0]}>
      ...
      {/* splayed back leg */}
      <mesh position={[0, 3.0, -1.0]} rotation={[0.32, 0, 0]} castShadow>
        <boxGeometry args={[0.16, 6.2, 0.16]} />
        <meshStandardMaterial color={WOOD} roughness={0.55} />
      </mesh>
      <mesh position={[0, 1.0, -0.5]} rotation={[0.5, 0, 0]}>
      ...
      <group position={[0, 4.0, 0.28]} rotation={[0.06, 0, 0]}>
        <mesh position={[0, 0, -0.05]} castShadow>
          <boxGeometry args={[2.5, 3.0, 0.08]} />
          <meshStandardMaterial color={LINEN} roughness={0.95} />
        </mesh>
        <mesh position={[0, 0, 0.01]}>
          <planeGeometry args={[2.34, 2.84]} />
      ...
      <group position={[-0.95, 2.1, 0.5]} rotation={[1.2, 0.2, 0.3]}>
        <mesh>
          <cylinderGeometry args={[0.46, 0.46, 0.03, 22]} />
          <meshStandardMaterial color="#9c7548" roughness={0.5} />
        </mesh>
        {["#b5472f", "#c8a23a", "#3f6079", "#6f8a5a", "#e6dcc2", "#8a3b50"].map(
          (c, i) => {
            const a = (i / 6) * Math.PI * 1.4 - 0.7
            return (
              <mesh
                key={i}
                position={[Math.cos(a) * 0.28, 0.025, Math.sin(a) * 0.28]}
              >
...
function PottersWheel({
  ...
  onClick,
  position = WHEEL_POS,
}: InteractiveProps & { position?: [number, number, number] }) {
  const [hovered, setHovered] = useState(false)
  const hoverP = useRef(0)
  const glowMats = useRef<(THREE.MeshStandardMaterial | null)[]>([])
  const lightRef = useRef<THREE.PointLight>(null!)
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 0.78, -0.35]} castShadow>
        ...
        <meshStandardMaterial color="#2b2f35" metalness={0.5} roughness={0.5} />
      </mesh>

      {/* two-piece splash pan */}
      <mesh position={[0, 1.32, 0]}>
        <cylinderGeometry args={[0.95, 0.9, 0.34, 32, 1, true]} />
        <meshStandardMaterial
          color="#cfd2d6"
          metalness={0.1}
          roughness={0.6}
      ...
      <mesh position={[0, 1.49, 0]}>
        <torusGeometry args={[0.92, 0.05, 12, 32]} />
        <meshStandardMaterial color="#b6bac0" roughness={0.5} />
      </mesh>
      <mesh position={[0, 1.17, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[0.34, 0.95, 32]} />
        <meshStandardMaterial
          color="#c4c8cd"
          roughness={0.6}
          side={THREE.DoubleSide}
      ...
      {[0.12, 0.22, 0.3].map((r, i) => (
        <mesh key={i} position={[0, 1.4, 0]}>
          <torusGeometry args={[r, 0.005, 6, 28]} />
          <meshStandardMaterial
            color="#7d838b"
            metalness={0.6}
            roughness={0.4}
      ...
      {/* foot pedal + cord */}
      <mesh position={[0.9, 0.06, 0.9]} rotation={[0, -0.4, 0.04]} castShadow>
        <boxGeometry args={[0.6, 0.1, 0.36]} />
        <meshStandardMaterial color="#2b2f35" metalness={0.4} roughness={0.6} />
      </mesh>
      ...
      <group position={[1.55, 0, 0.3]}>
        <mesh position={[0, 0.4, 0]} castShadow>
          <cylinderGeometry args={[0.3, 0.26, 0.8, 18]} />
          <meshStandardMaterial color="#7d8c6a" roughness={0.6} />
        </mesh>
        <mesh position={[0, 0.78, 0]}>
          <cylinderGeometry args={[0.27, 0.27, 0.03, 18]} />
          <meshStandardMaterial
            color="#3a5a6e"
            transparent
            opacity={0.7}
        ...
        <mesh position={[0.06, 0.82, 0]}>
          <boxGeometry args={[0.18, 0.09, 0.14]} />
          <meshStandardMaterial color="#d8c98a" roughness={1} />
        </mesh>
      </group>
    ...
    <group position={TELE_POS}>
      ...
      {[0, 1, 2].map((i) => {
        ...
        return (
          <group key={i}>
            <mesh
              position={[Math.cos(a) * 0.95, 1.45, Math.sin(a) * 0.95]}
            ...
            <mesh
              position={[Math.cos(a) * 0.55, 0.9, Math.sin(a) * 0.55]}
              rotation={[0, -a, 0]}
            >
      ...
      <mesh position={[0, 3.25, 0]} rotation={[0.5, 0, 0]} castShadow>
      ...
      <mesh position={[0.28, 3.0, 0.2]} rotation={[0.3, 0, -0.5]}>
      ...
      <mesh position={[-0.55, 2.78, -0.32]} castShadow>
        <cylinderGeometry args={[0.16, 0.16, 0.2, 18]} />
        <meshStandardMaterial
          color={METAL_DARK}
          metalness={0.7}
          roughness={0.35}
      ...
      <group position={[0, 3.55, 0]} rotation={[-0.7, 0.35, 0]}>
        ...
        {[-0.25, 0.25].map((y, i) => (
          <mesh key={i} position={[0, y, 0]}>
            <torusGeometry args={[0.185, 0.035, 10, 28]} />
            <meshStandardMaterial
              color={METAL_DARK}
              metalness={0.6}
              roughness={0.4}
        ...
        <mesh position={[0, 1.78, 0]}>
          <cylinderGeometry args={[0.205, 0.18, 0.55, 32]} />
          <meshStandardMaterial
            color="#c7a25e"
            metalness={0.85}
            roughness={0.28}
        ...
        <mesh position={[0, 1.74, 0]}>
          <torusGeometry args={[0.185, 0.022, 10, 32]} />
          <meshStandardMaterial
            color="#8a6a32"
            metalness={0.8}
            roughness={0.32}
        ...
        {[-0.9, 0, 0.9].map((y, i) => (
          <mesh key={i} position={[0, y, 0]}>
            <torusGeometry args={[0.173, 0.018, 8, 28]} />
            <meshStandardMaterial
              color="#8a6a32"
              metalness={0.8}
              roughness={0.35}
        ...
        <mesh position={[0, -1.78, 0]}>
          <cylinderGeometry args={[0.14, 0.17, 0.45, 20]} />
          <meshStandardMaterial
            color={METAL_DARK}
            metalness={0.6}
            roughness={0.32}
        ...
        {[0.17, -0.17].map((x, i) => (
          <mesh key={i} position={[x, -1.7, 0]} rotation={[0, 0, Math.PI / 2]}>
        ...
        <mesh position={[0.2, -2.18, 0]} rotation={[0, 0, -Math.PI / 2]}>
          <cylinderGeometry args={[0.07, 0.08, 0.24, 16]} />
          <meshStandardMaterial
            color={METAL_DARK}
            metalness={0.6}
            roughness={0.32}
        ...
        <mesh position={[0.34, -2.18, 0]} rotation={[0, 0, -Math.PI / 2]}>
        ...
        <group position={[0.28, 0.35, 0]}>
          <mesh rotation={[0, 0, 0.04]}>
            <cylinderGeometry args={[0.045, 0.045, 1.0, 14]} />
            <meshStandardMaterial
              color={METAL_DARK}
              metalness={0.6}
              roughness={0.4}
      ...
      <group position={[1.4, 0, 0.9]}>
        ...
        <mesh position={[0, 1.06, 0]} rotation={[-Math.PI / 2, 0, 0.2]}>
          <planeGeometry args={[0.95, 0.72]} />
          <meshStandardMaterial color="#10243f" roughness={0.85} />
        </mesh>
        {[
          [-0.2, 0.1],
          [0.1, -0.15],
...
function DoorLock({ unlocked, roomPhase, onDoorClick }: DoorLockProps) {
  ...
  const shacklePos: [number, number, number] = unlocked
    ? [0.55, 1.1, 0]
    : [0, 0.78, 0]
  const shackleRot: [number, number, number] = unlocked
    ? [0, 0, 0.65]
    : [0, 0, 0]
  ...
  return (
    <group position={DOOR_POS}>
      ...
      {[-1.1, 0, 1.1].map((x) => (
        <mesh key={x} position={[x, 0, -0.03]}>
          <boxGeometry args={[0.08, 6.2, 0.32]} />
          <meshStandardMaterial color={WOOD_DARK} roughness={0.85} />
        </mesh>
      ))}
      {[-1.8, 1.8].map((y) => (
        <mesh key={y} position={[0, y, 0]}>
          <boxGeometry args={[3.6, 0.25, 0.34]} />
          <meshStandardMaterial
            color={METAL_DARK}
            metalness={0.6}
            roughness={0.4}
      ...
      <group position={shacklePos} rotation={shackleRot}>
        <mesh position={[0, 0, 0.1]}>
          <torusGeometry args={[0.4, 0.13, 10, 20, Math.PI]} />
          <meshStandardMaterial
            color={col}
            emissive={col}
            emissiveIntensity={emissive}
