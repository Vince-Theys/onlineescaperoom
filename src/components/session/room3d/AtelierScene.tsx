...
const PLASTER    = "#ece6d8"   // casts / busts
const BRASS      = "#b3914f"
...
/** A classical white plaster bust on a plinth. */
function PlasterBust({ position, rotation = 0 }: { position: [number, number, number]; rotation?: number }) {
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      {/* shoulders / chest */}
      <mesh position={[0, 0.5, 0]} castShadow><cylinderGeometry args={[0.5, 0.62, 1.0, 20]} /><meshStandardMaterial color={PLASTER} roughness={0.92} /></mesh>
      <mesh position={[0, 0.0, 0]}><cylinderGeometry args={[0.4, 0.4, 0.2, 20]} /><meshStandardMaterial color="#dcd4c4" roughness={0.9} /></mesh>
      {/* neck + head */}
      <mesh position={[0, 1.12, 0.02]} castShadow><cylinderGeometry args={[0.17, 0.22, 0.36, 16]} /><meshStandardMaterial color={PLASTER} roughness={0.92} /></mesh>
      <mesh position={[0, 1.5, 0.02]} scale={[0.9, 1.1, 1]} castShadow><sphereGeometry args={[0.32, 24, 24]} /><meshStandardMaterial color={PLASTER} roughness={0.92} /></mesh>
      {/* nose + hair mass */}
      <mesh position={[0, 1.48, 0.32]} rotation={[Math.PI / 2, 0, 0]}><coneGeometry args={[0.06, 0.16, 8]} /><meshStandardMaterial color={PLASTER} roughness={0.92} /></mesh>
      <mesh position={[0, 1.66, -0.08]} scale={[1, 0.8, 1]}><sphereGeometry args={[0.34, 16, 16]} /><meshStandardMaterial color="#e3dccd" roughness={0.95} /></mesh>
    </group>
  )
}
...
function Stool({ position }: { position: [number, number, number] }) {
  return (
    <group position={position}>
      <mesh position={[0, 1.5, 0]} castShadow><cylinderGeometry args={[0.46, 0.46, 0.12, 18]} /><meshStandardMaterial color={WOOD} roughness={0.6} /></mesh>
      {[0, 1, 2].map((i) => {
        const a = (i / 3) * Math.PI * 2
        return (
          <mesh key={i} position={[Math.cos(a) * 0.32, 0.74, Math.sin(a) * 0.32]} rotation={[Math.cos(a) * 0.12, 0, -Math.sin(a) * 0.12]}>
            <cylinderGeometry args={[0.045, 0.045, 1.5, 8]} /><meshStandardMaterial color={WOOD_DARK} roughness={0.6} />
          </mesh>
        )
      })}
      <mesh position={[0, 0.7, 0]} rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[0.3, 0.02, 6, 18]} /><meshStandardMaterial color={WOOD_DARK} roughness={0.6} /></mesh>
    </group>
  )
}
...
function PotteryShelf({ position, rotation = 0 }: { position: [number, number, number]; rotation?: number }) {
  const pots = [
    { x: -1.4, c: TERRA,     r: 0.22, h: 0.5 },
    { x: -0.7, c: "#cdbb9a", r: 0.18, h: 0.36 },
    { x: 0.0,  c: "#3a6e7d", r: 0.26, h: 0.6 },
    { x: 0.75, c: "#9c5e3c", r: 0.16, h: 0.44 },
    { x: 1.4,  c: "#7d8c6a", r: 0.2,  h: 0.34 },
  ]
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      {pots.map((p, i) => (
        <group key={i} position={[p.x, 0.05 + p.h / 2, 0]}>
          <mesh castShadow><cylinderGeometry args={[p.r * 0.7, p.r, p.h, 20]} /><meshStandardMaterial color={p.c} roughness={0.55} metalness={0.05} /></mesh>
          <mesh position={[0, p.h / 2, 0]}><torusGeometry args={[p.r * 0.7, 0.02, 8, 20]} /><meshStandardMaterial color={p.c} roughness={0.5} /></mesh>
        </group>
      ))}
...
function AtelierDecor() {
  return (
    <group>
      ...
      {/* plaster casts on pedestals + a shelf bust */}
      <group position={[-9, 0, -6]}>
        <mesh position={[0, 1.4, 0]} castShadow><boxGeometry args={[1.0, 2.8, 1.0]} /><meshStandardMaterial color="#e8e2d4" roughness={0.85} /></mesh>
        <mesh position={[0, 2.86, 0]}><boxGeometry args={[1.2, 0.14, 1.2]} /><meshStandardMaterial color="#f2ede1" roughness={0.8} /></mesh>
        <PlasterBust position={[0, 2.95, 0]} rotation={0.3} />
      </group>
      ...
      {/* a couple of crumpled studies near the easel */}
      <mesh position={[-9, 0.16, 4]} rotation={[0.4, 1.1, 0.7]} castShadow><icosahedronGeometry args={[0.16, 0]} /><meshStandardMaterial color="#e7dcc4" roughness={1} flatShading /></mesh>
      <mesh position={[-8.4, 0.16, 4.6]} rotation={[1.2, 0.3, 2.1]} castShadow><icosahedronGeometry args={[0.14, 0]} /><meshStandardMaterial color="#e2d6bc" roughness={1} flatShading /></mesh>
    </group>
  )
}
...
function PottersWheel({ active, completed, onClick }: InteractiveProps) {
  const [hovered, setHovered] = useState(false)
  const hoverP = useRef(0)
  const glowMats = useRef<(THREE.MeshStandardMaterial | null)[]>([])
  const lightRef = useRef<THREE.PointLight>(null!)
  ...
  return (
    <group position={WHEEL_POS}>
      ...
      {/* clay being thrown — a half-formed bowl with throwing rings + spiral */}
      <group position={[0, 1.42, 0]}>
        <mesh castShadow><cylinderGeometry args={[0.3, 0.26, 0.18, 28]} /><meshStandardMaterial color={TERRA} roughness={0.9} /></mesh>
        <mesh position={[0, 0.22, 0]}><cylinderGeometry args={[0.26, 0.3, 0.32, 28, 1, true]} /><meshStandardMaterial color="#bb764e" roughness={0.9} side={THREE.DoubleSide} /></mesh>
        <mesh position={[0, 0.36, 0]}><torusGeometry args={[0.26, 0.02, 8, 28]} /><meshStandardMaterial color="#9c5e3c" roughness={0.92} /></mesh>
        {[0.06, 0.16, 0.26].map((y, i) => (<mesh key={i} position={[0, y, 0]}><torusGeometry args={[0.3 - i * 0.02, 0.008, 6, 28]} /><meshStandardMaterial color="#9c5e3c" roughness={0.92} /></mesh>))}
      </group>
